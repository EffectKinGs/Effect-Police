...
async def send_log(
    ...
    *,
    ...
    try:
        ...
        routed = await db.get_setting(f"logs_channel_ids:{guild.id}:{slug}")
        channels = []
        if routed:
            channels = [c for part in routed.split(",") if (c := guild.get_channel(int(part.strip())) if part.strip().isdigit() else None) and isinstance(c, discord.TextChannel)]
        if not channels:
