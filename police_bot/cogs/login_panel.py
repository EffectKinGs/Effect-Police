"""لوحة Effect King's Police للمباشرة مع استبيان Login ووضع الأزرار الموسع."""
...
async def refresh_duty_panel(guild: discord.Guild, expanded: bool = False) -> bool:
    ...
    try:
        ...
        await utils.edit_panel(message, duty_embed, LoginPanelView(expanded=expanded), config.LOGIN_BANNER_ASSET)
        return True
...
class LoginPanel(commands.Cog):
    ...
    async def publish_panel(self, interaction: discord.Interaction):
        ...
        if existing_id and await db.get_setting(_guild_key(PANEL_CHANNEL_KEY, interaction.guild.id)) == str(interaction.channel.id):
            try:
                ...
                await utils.edit_panel(message, embed, LoginPanelView(expanded=False), config.LOGIN_BANNER_ASSET)
                await interaction.delete_original_response()
                return
            except (discord.NotFound, discord.Forbidden, discord.HTTPException, ValueError):
                pass

        message = await utils.send_panel(interaction.channel, embed, LoginPanelView(expanded=False), config.LOGIN_BANNER_ASSET)
    ...
    @commands.command(name="login-on")
    async def open_login(self, ctx: commands.Context):
        if not ctx.guild or not isinstance(ctx.author, discord.Member) or not utils.has_role(ctx.author, "admin"):
            await ctx.send("-# **<:emoji_28:1550309826758840500>  '  ليست من صلاحياتك نشرها **")
            return
        await db.set_login_enabled(True)
        await refresh_duty_panel(ctx.guild)
        await utils.send_log(ctx.guild, "Login Opened", f"المنفذ: {ctx.author.mention}")
        await ctx.send("** <:emoji_5:1550308058448527472>  '  تم فتح تسجيل الدخول . **")

    @commands.command(name="login-off")
    async def close_login(self, ctx: commands.Context):
        if not ctx.guild or not isinstance(ctx.author, discord.Member) or not utils.has_role(ctx.author, "admin"):
            await ctx.send("-# **<:emoji_28:1550309826758840500>  '  ليست من صلاحياتك نشرها **")
            return
        await db.set_login_enabled(False)
        await refresh_duty_panel(ctx.guild)
        await utils.send_log(ctx.guild, "Login Closed", f"المنفذ: {ctx.author.mention}")
        await ctx.send("**<:emoji_6:1550308073531252878>  '  تم قفل تسجيل الدخول . **")
