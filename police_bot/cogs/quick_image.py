...
IMAGE_PATH = Path(__file__).resolve().parent.parent / "assets" / "quick_x.png"
...
class QuickImage(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.channel.id in config.LINE_IMAGE_AUTO_CHANNEL_IDS:
            if any(attachment.filename == "quick_x.png" for attachment in message.attachments):
                return
    ...
    async def _send_image(self, channel: discord.abc.Messageable) -> bool:
        if not IMAGE_PATH.is_file():
            ...
            return False
        try:
            await channel.send(file=discord.File(IMAGE_PATH, filename="quick_x.png"))
            return True
