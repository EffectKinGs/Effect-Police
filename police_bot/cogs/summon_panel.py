...
def _can_summon(member: discord.Member) -> bool:
    if member.guild_permissions.administrator:
        return True
    if utils.has_role(member, "admin"):
        return True
...
class SummonPanel(commands.Cog):
    ...
    async def summon_panel(self, interaction: discord.Interaction):
        ...
        embed = utils.base_embed("EvilTown ( Summon )", description, image_url="attachment://evil_town_banner.png")
        await utils.send_panel(interaction.channel, embed, SummonPanelView(), "evil_town_banner.png")
