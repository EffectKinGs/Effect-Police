"""أوامر الإدارة والتهيئة العامة لنظام Effect King's Police."""
...
class SetupAdmin(commands.Cog):
    ...
    @app_commands.describe(log_type="all أو اسم العملية مثل login أو ticket_created")
    async def set_log_channel(self, interaction: discord.Interaction, log_type: str = "all"):
        ...
        key = f"logs_channel_ids:{interaction.guild.id}" if log_type == "all" else f"logs_channel_ids:{interaction.guild.id}:{log_type.lower().replace(' ', '_')}"
        raw = await db.get_setting(key) or ""
        ids = {p.strip() for p in raw.split(",") if p.strip()}
        ids.add(str(interaction.channel.id))
        await db.set_setting(key, ",".join(ids))
        await utils.send_log(interaction.guild, "Logs Enabled", f"تم إضافة هذه القناة لقنوات اللوقات بواسطة {interaction.user.mention}. النوع: {log_type}")
    ...
    @app_commands.default_permissions(administrator=True)
    @app_commands.describe(log_type="all أو اسم العملية مثل login أو ticket_created")
    async def remove_log_channel(self, interaction: discord.Interaction, log_type: str = "all"):
        if interaction.guild is None or not utils.has_role(interaction.user, "admin") or not isinstance(interaction.channel, discord.TextChannel):
            await interaction.response.send_message("❌ هذا الأمر للمسؤولين وفي قناة نصية فقط.", ephemeral=True)
            return
        await interaction.response.defer(ephemeral=True)
    ...
    async def help_command(self, interaction: discord.Interaction):
        embed = utils.base_embed("📖 Effect King's")
